...
public class LotService {
    ...
    //LOT 코드 생성규칙1
    public LotDto ruleProductName(String processId, String wplanId){
        ...
        lotDto.setPorderId("-"); //발주 고유번호
        lotDto.setArrivalDate(LocalDateTime.now()); //원자재 입고일
    ...
    public void createLotRecode(String processId, String wplanId){
       String checkWorder = worderService.checkWorder(processId, wplanId); //공정고유번호를 체크해서 현재 가동상태가 완료인 것(=작업완료시간이 현재시간 이전이면)
        ...
        if(checkWorder == "완료"){
            LotDto result  = ruleProductName(processId, wplanId);
